...
def mean(prices: list[float], period: int) -> list[float]:
    """
...
def median(prices: list[float], period: int) -> list[float]:
    """
...
def variance(prices: list[float], period: int) -> list[float]:
    """
    ...
    variances = []
    for i in range(period, len(prices)+1):
        variances.append(statistics.variance(prices[i - period:i]))
    return variances
