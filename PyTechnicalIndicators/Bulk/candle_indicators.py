...
def bollinger_bands(typical_price):
...
def personalised_bollinger_bands(typical_price, period, ma_model=None):
    ...
    for i in range(period, len(typical_price)):
        if ma_model in ma:
            ...
            ma = smoothed_moving_average(typical_price[i - 20:i])
        elif ma_model in ema:
            ma = exponential_moving_average(typical_price[i - 20:i])
        else:
            ma = moving_average(typical_price[i - 20:i])
